...
| `streamed` | bool |
| `outcome` | `completed` \| `client_abandoned` \| `upstream_error` \| `cancelled_queued` \| `rejected_unadmittable` (the request's admission cost exceeds its pool's total budget, so it can never be admitted — see `cost_exceeds_pool_budget` below; rejected at admission, no slot/budget taken, #36) |
...
| `cached_tokens` | prompt tokens the backend served from its prefix/KV cache — `usage.prompt_tokens_details.cached_tokens` (OpenAI, DeepSeek, antirez/ds4, vLLM, llama.cpp). `NULL` = backend has no cache accounting; an explicit `0` is a reported miss. Surfaces as `cache hit %` per model (`/models`, over calls that report it) and as `cached tok` per consumer / workload / request |
